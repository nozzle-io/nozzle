...
struct DirectoryEntry {
    ...
    uint8_t backend{0};
    pid_t pid{0};
    uint32_t valid{0}; // 1 = active, 0 = tombstone
};
...
struct SenderSharedState {
    ...
    // Ring buffer state — _Atomic for cross-process visibility via cache coherency
    alignas(64) _Atomic uint64_t committed_frame;
    alignas(64) _Atomic uint32_t committed_slot;

    struct SlotInfo {
        uint64_t frame_number{0};
        uint32_t iosurface_id{0};  // IOSurfaceGetID() result on macOS
        uint32_t shared_handle{0}; // Lower 32 bits of HANDLE on Windows
    } slots[kMaxRingSlots]{};
