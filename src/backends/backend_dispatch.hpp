...
auto lookup_texture(void *device, uint64_t shared_id, uint32_t width, uint32_t height, uint32_t format, uint8_t channel_swizzle, uint32_t semantic_format = 0, uint64_t native_modifier = 0) -> Result<texture>;
