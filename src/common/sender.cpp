...
Result<void> sender::publish_native_texture(void *native_texture, uint32_t width, uint32_t height, texture_format format, texture_format semantic_format) {
	if (!impl_ || !impl_->valid_) {
		return Error{ErrorCode::BackendError, "sender is not valid"};
	}
	if (!native_texture) {
	...
	if (detail::backend::is_native_texture_shared(native_texture)) {
		...
		if (surface) {
			...
			if (resource_id != detail::kInvalidSharedResourceId) {
				...
				impl_->state->slots[slot].frame_number = frame_number;
				impl_->state->slots[slot].shared_resource_id = resource_id;
				impl_->state->slots[slot].width = width;
				impl_->state->slots[slot].height = height;
			impl_->state->slots[slot].format = static_cast<uint32_t>(wrapped.desc().format);
			impl_->state->slots[slot].semantic_format = static_cast<uint32_t>(semantic_format);
		...
		impl_->state->semantic_format = static_cast<uint32_t>(semantic_format);
